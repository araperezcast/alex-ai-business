...
export function SlaBadge({ op }: { op: Operation }) {
  ...
  return (
    ...
    >
      ...
      {op.status === "pending"
        ? `${meta.label} · ${hours.toFixed(1)}h`
        : `Answered in ${hours.toFixed(1)}h`}
    </span>
  );
}
...
export function PulseView({ rows }: { rows: Operation[] }) {
  const premium = rows
...
export function CatalogsView({ rows }: { rows: Operation[] }) {
...
export function QuoteCaptureModal({
  ...
  useEffect(() => {
    if (operation) {
      setQuotes([operation.quotes[0] ?? EMPTY_QUOTE, operation.quotes[1] ?? EMPTY_QUOTE]);
    }
  }, [operation]);
                ...
                <div className="space-y-2 md:col-span-2">
                  ...
                  <label className="flex cursor-pointer items-center gap-3 rounded-xl border border-dashed border-slate-300 bg-slate-50/60 px-4 py-3 text-sm text-slate-500">
                    ...
                    <input
                      ...
                      accept=".pdf"
                      className="hidden"
                      onChange={(e) => patch(i, "internalFile", e.target.files?.[0]?.name ?? "")}
                    />
                  </label>
                </div>
              </div>
